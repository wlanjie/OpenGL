...
ScaleShader::ScaleShader(int width, int height) {
    this->width = width;
    this->height = height;
    progress = 0;
    frames = 0;
...
GLuint ScaleShader::processImage(int textureId) {
    shader->use();
    glBindFramebuffer(GL_FRAMEBUFFER, frameBufferId);
    ...
    auto offsetXUniform = shader->uniformIndex("offsetX");
    auto offsetYUniform = shader->uniformIndex("offsetY");
    glUniform1f(scaleUniform, 0.1);
    glUniform1f(offsetXUniform, 0.2);
    glUniform1f(offsetYUniform, 0.0);
